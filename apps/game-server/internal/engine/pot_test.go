...
func TestPot_RemainderDistribution(t *testing.T) {
	...
	// 400 / 3 = 133 rem 1 -> one winner gets 134, others get 133
	for pid, amt := range result {
		if amt != 133 && amt != 134 {
			t.Errorf("expected %s to win 133 or 134, got %d", pid, amt)
		}
	}
	if result["p1"]+result["p2"]+result["p3"] != 400 {
		t.Errorf("expected total 400, got %d", result["p1"]+result["p2"]+result["p3"])
	}
}
